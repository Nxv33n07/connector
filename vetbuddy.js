...
async function getToken() {
  ...
  if (!res.data?.Token) throw new Error('VetBuddy token fetch failed: ' + JSON.stringify(res.data));
...
async function apiGet(params, timeout = 30000, isRetry = false) {
  ...
  if (dataStr.includes('Invalid Token')) {
    console.warn(`[VetBuddy] Warning: Received 'Invalid Token'. ${isRetry ? 'Aborting.' : 'Clearing token cache and retrying...'}`);
    if (!isRetry) {
      _token = null; // Invalidate memory cache
      _expiresAt = 0;
      return apiGet(params, timeout, true); // Retry once with fresh token
    }
    throw new Error('VetBuddy API request failed with persistent Invalid Token error.');
  }
...
const getMedicalRecords = (p={}) => fetchAll({ action: 'patientmrlist',  ...p });
const getPatientDx      = (p={}) => fetchAll({ action: 'patientdx',      ...p });
const getAppointmentTypes=(p={}) => fetchAll({ action: 'appointment_type',...p });

async function getStaffAvailability(availabilitydate, clinicid) {
  const res = await apiGet({ action: 'staffavailability', availabilitydate, clinicid }, 15000);
...
function parseResponse(text) {
  ...
  return { success: p[0] === 'OK', id: p[1] || null, action: p[2] || null, raw: text };
}
async function pushXML(slug, xml) {
  const url = `${PUSH_BASE}/wsapi.html?instance=${INSTANCE}&key=${PUSH_KEY}&slug=${slug}`;
  const res = await axios.post(url, xml, { headers: { 'Content-Type': 'application/xml' }, timeout: 15000, responseType: 'text' });
  return parseResponse(res.data);
}
const cd = v => `<![CDATA[${v||''}]]>`;

async function createClient(c) {
  return pushXML('client', `<?xml version="1.0" encoding="UTF-8"?>
...
async function createPatient(p) {
  return pushXML('patient', `<?xml version="1.0" encoding="UTF-8"?>
...
async function createAppointment(a) {
  return pushXML('appointment', `<?xml version="1.0" encoding="UTF-8"?>
...
module.exports = {
  getToken, getClinics, getClients, getPatients, getAppointments, getInvoices,
  getPayments, getStock, getStaff, getReminders, getStaffRota, getMedicalRecords,
  getPatientDx, getAppointmentTypes, getStaffAvailability, getClientAccountSummary,
  createClient, updateClient, createPatient, updatePatient, createAppointment, cancelAppointment,
};
